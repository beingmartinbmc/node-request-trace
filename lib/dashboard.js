...
function renderTimeline(steps, totalDur) {
  ...
  return steps.map((s, i) => {
    ...
    return '<div class="tl-row"><div class="tl-label">' + s.name + '</div>'
      ...
      + '<div class="tl-dur">' + s.duration + 'ms</div></div>';
  }).join('');
}
...
async function showDetail(id) {
  ...
  p.innerHTML = '<button class="close-btn" onclick="document.getElementById(\\'detail-overlay\\').classList.remove(\\'open\\')">&times;</button>'
    ...
    + '</div>'
    + '<div class="section-title">Timeline (' + (t.steps ? t.steps.length : 0) + ' steps)</div>'
    + '<div class="timeline">' + renderTimeline(t.steps, t.duration) + '</div>';
