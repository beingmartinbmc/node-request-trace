...
async function exportDownload(id, suffix, ext, mime) {
  const r = await fetch('/trace/' + id + '/' + suffix);
  if (!r.ok) return;
  const data = await r.text();
  const blob = new Blob([data], { type: mime });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'trace-' + id + '.' + ext;
  a.click();
  URL.revokeObjectURL(a.href);
}

async function copyMarkdown(id) {
  const r = await fetch('/trace/' + id + '/markdown');
  if (!r.ok) return;
...
async function showDetail(id) {
  ...
  p.innerHTML = '<button class="close-btn" onclick="document.getElementById(\\'detail-overlay\\').classList.remove(\\'open\\')">&times;</button>'
    ...
    + '<button class="export-btn" onclick="exportDownload(\\'' + t.requestId + '\\',\\'speedscope\\',\\'speedscope.json\\',\\'application/json\\')">\\u2B07 Speedscope</button>'
    + '<button class="export-btn" onclick="exportDownload(\\'' + t.requestId + '\\',\\'snapshot\\',\\'html\\',\\'text/html\\')">\\u2B07 Snapshot</button>'
    + '<button class="export-btn" onclick="copyMarkdown(\\'' + t.requestId + '\\')">\\u29C9 Copy Markdown</button>'
    + '</h2>'
